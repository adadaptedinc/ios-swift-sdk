...
//  AAObjCExampleApp
//
//  Created by Brett Clifton on 8/7/20.
//  Copyright © 2020 AdAdapted. All rights reserved.
//

import UIKit

@objc open class ObjCSearchTextField: UITextField {
    ...
    override open func willMove(toSuperview newSuperview: UIView?) {
        ...
        self.addTarget(self, action: #selector(ObjCSearchTextField.textFieldDidChange), for: .editingChanged)
        self.addTarget(self, action: #selector(ObjCSearchTextField.textFieldDidBeginEditing), for: .editingDidBegin)
        self.addTarget(self, action: #selector(ObjCSearchTextField.textFieldDidEndEditing), for: .editingDidEnd)
        self.addTarget(self, action: #selector(ObjCSearchTextField.textFieldDidEndEditingOnExit), for: .editingDidEndOnExit)
        
        NotificationCenter.default.addObserver(self, selector: #selector(ObjCSearchTextField.keyboardWillShow(_:)), name: UIResponder.keyboardWillShowNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(ObjCSearchTextField.keyboardWillHide(_:)), name: UIResponder.keyboardWillHideNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(ObjCSearchTextField.keyboardDidChangeFrame(_:)), name: UIResponder.keyboardDidChangeFrameNotification, object: nil)
    }
    ...
    @objc open func textFieldDidChange() {
        ...
        timer?.invalidate()
        timer = Timer.scheduledTimer(timeInterval: typingStoppedDelay, target: self, selector: #selector(ObjCSearchTextField.typingDidStop), userInfo: self, repeats: false)
        ...
        for i in 0 ..< filterDataSource.count {
            ...
            if !inlineMode {
                ...
                if titleFilterRange.location != NSNotFound || subtitleFilterRange.location != NSNotFound || addAll {
                    ...
                    let interceptSuggestion = KeywordInterceptor.init().getInterceptSuggestions(text!)
                    if interceptSuggestion != nil {
                        getInterceptSuggestions(suggestion: interceptSuggestion!)
                    }
    ...
    fileprivate func getInterceptSuggestions(suggestion: String) {
       let interceptItem = SearchTextFieldItem(title: suggestion)
        interceptItem.attributedTitle = NSMutableAttributedString(string: suggestion)
        interceptItem.attributedTitle!.setAttributes(highlightAttributes, range: (interceptItem.title as NSString).range(of: suggestion, options: comparisonOptions))
...
extension ObjCSearchTextField: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        var cell = tableView.dequeueReusableCell(withIdentifier: ObjCSearchTextField.cellIdentifier)
        
        if cell == nil {
            cell = UITableViewCell(style: .subtitle, reuseIdentifier: ObjCSearchTextField.cellIdentifier)
        }
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if itemSelectionHandler == nil {
            ...
            if selectedItem.isSponsored {
                KeywordInterceptor.init().interceptWasSelected()
            }
